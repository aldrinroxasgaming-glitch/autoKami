...
async function main() {
  ...
  const recipeIndex = 6;
  const amount = 10;
  const specialAssignerID = '0x5c817c70'; // Provided magic value
  ...
  try {
    ...
    for (const wallet of operatorWallets) {
        ...
        try {
            ...
            // Execute craft
            console.log(`Executing craft transaction with Account ID: ${wallet.account_id}...`);
            const tx = await executeSystemCall({
                systemId: 'system.craft',
                typedParams: [
                    wallet.account_id, // assignerID (uint256) - Using Account ID
